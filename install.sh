...
claude=0 hermes=0 oc=0 ds=0 agy=0 explicit=0 uninstall=0
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --all)            claude=1; hermes=1; oc=1; ds=1; agy=1; explicit=1 ;;
    -n|--dry-run)     LIB_DRY_RUN=1 ;;
    -f|--force)       LIB_FORCE=1 ;;
    --uninstall)      uninstall=1 ;;
    -h|--help)        sed -n '2,15p' "$0"; exit 0 ;;
    *) echo "unknown flag: $1" >&2; sed -n '2,15p' "$0" >&2; exit 2 ;;
  esac
  shift
done

[ "$explicit" -eq 0 ] && { claude=1; hermes=1; oc=1; ds=1; agy=1; }
...
if [ "$uninstall" -eq 0 ]; then
  lib_validate_skills || \
    printf '       (linking anyway; trim the description or these skills will not load in Claude Code)\n' >&2
fi
...
[ "$agy"    -eq 1 ] && $action "agy"            "$HOME/.gemini/antigravity-cli"
